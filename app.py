...
APP_TITLE = "FRANCE INSIDE SALES ACCOUNTS"
...
def get_data(IS_name):
    ...
    report_id_accounts = '00OQv00000CcL2vMAF'
    report_id_visits = '00OQv00000CcMJxMAN'
...
def main():
    ...
    with cola:
        ...
        with col1:
            hfr_list = df['HFR'].map(lambda x: str(x)).unique()
            for i, n in enumerate(hfr_list):
                if n == "nan":
                    hfr_list[i] = "-"
            hfr_list.sort()
            hfr = st.multiselect('Hunting/Farming/Refocus', hfr_list)
    ...
    df_filtered = df[(df['HFR'].isin(hfr_filter ))
